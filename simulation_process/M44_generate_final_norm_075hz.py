"""
Utility for producing final M44 "clean" datasets and their normalized copies.

Pipeline:
  1. Load every file in processed_M44/m44_256_128 (shape [3, 256, 128, T]).
  2. Apply a 1 Hz FFT low-pass filter (fs=4 Hz) and save the filtered volume
     without changing its resolution to final_norm/m44_256_128_clean.
  3. Apply a 0.75 Hz low-pass filter to the same input, downsample to
     [3, 128, 64, 48], and save the volumes to
     final_norm/m44_128_64_48_fmax_075_clean.
  4. Normalize all newly created "clean" files with the method used in
     norm_all_M7_M44.py (divide by std, append log10(std) channel) and
...
BASE_PROCESSED = Path(os.getenv("GMFLOW_PROCESSED_ROOT", "path/to/your/processed_output")) / "processed_M44"
INPUT_DIR = BASE_PROCESSED / "m44_256_128"
FINAL_NORM_BASE = BASE_PROCESSED / "final_norm"
CLEAN_FULL_DIR = FINAL_NORM_BASE / "m44_256_128_clean"
CLEAN_DOWNSAMPLED_48_DIR = FINAL_NORM_BASE / "m44_128_64_48_fmax_075_clean"
NORMALIZED_BASE = FINAL_NORM_BASE / "normalized"
NORM_FULL_DIR = NORMALIZED_BASE / "norm_m44_256_128_clean"
NORM_DOWNSAMPLED_48_DIR = NORMALIZED_BASE / "norm_m44_128_64_48_fmax_075_clean"
...
def process_single_file(
    ...
    full_clean_path = CLEAN_FULL_DIR / f"m44_clean_256_128_sim_{sim_id}.npy"
    clean_48_path = (
        CLEAN_DOWNSAMPLED_48_DIR / f"m44_clean_128_64_48_fmax_075_sim_{sim_id}.npy"
    )
...
def generate_clean_datasets(
    ...
    for file_path in tqdm(files, desc="Filtering M44 data"):
        try:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate filtered + normalized M44 datasets.")
    parser.add_argument(
        "mode",
        nargs="?",
        default="all",
