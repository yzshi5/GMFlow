...
M7_PROC_320_160_DIR = M7_PROCESSED_DIR / 'm7_320_160'
M7_PROC_256_128_DIR = M7_PROCESSED_DIR / 'm7_256_128'
...
data_path_M7 = M7_PROCESSED_RAW_DIR

save_path_M7_320_160 = M7_PROC_320_160_DIR
save_path_M7_256_128 = M7_PROC_256_128_DIR
...
def process_all():
    ...
    os.makedirs(save_path_M7_320_160, exist_ok=True)
    os.makedirs(save_path_M7_256_128, exist_ok=True)
    # original data shape is [3, 367, 171, 118] 
    files = [
        f for f in os.listdir(data_path_M7)
        if f.endswith('.npy') and not f.startswith('._') and not f.startswith('.')
    ]
    files.sort()
    for file in tqdm(files):
        data = np.load(os.path.join(data_path_M7, file), allow_pickle=False)
        ...
        # save the data with standardized names: m7_proc_r_*_sim_{idx}.npy (float32)
        stem = Path(file).stem  # e.g., m7_raw_r_sim_0
        sim_str = "unknown"
        if "_sim_" in stem:
        ...
        out_320 = f"m7_proc_r_320_160_sim_{sim_str}.npy"
        out_256 = f"m7_proc_r_256_128_sim_{sim_str}.npy"
        np.save(save_path_M7_320_160 / out_320, data.astype(np.float32, copy=False))
        np.save(save_path_M7_256_128 / out_256, data_256.astype(np.float32, copy=False))


def test_single_sample(
    raw_dir: Path = data_path_M7,
    out_320_dir: Path = save_path_M7_320_160,
    out_256_dir: Path = save_path_M7_256_128,
    sim_idx: int = 0,
):
    """
    Process one sample:
    ...
    raw_name = f"m7_raw_r_sim_{sim_idx}.npy"
    ...
    out_name_320 = f"m7_proc_r_320_160_sim_{sim_idx}.npy"
    out_name_256 = f"m7_proc_r_256_128_sim_{sim_idx}.npy"
...
if __name__ == "__main__":
    ...
    if mode == "run":
        ...
        print("Usage:")
        print("  python rupture_post_process.py run")
        print("  python rupture_post_process.py test [sim_idx]")
