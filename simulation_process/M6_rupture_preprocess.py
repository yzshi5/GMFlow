...
M6_RESULTS_DIR = RAW_SIM_ROOT / 'final_results_M6'
M6_SUMMARY_CSV = RAW_SIM_ROOT / 'M6_rupture_realization_summary.csv'
M6_PROCESSED_DIR = PROCESSED_ROOT / 'processed_M6'
M6_PROCESSED_RAW_DIR = M6_PROCESSED_DIR / 'raw'
M6_PLOTS_DIR = PROCESSED_ROOT / 'temp_plot' / 'M6'
...
def process_rupture_data(start_idx: int = 0, max_count: int | None = None):
    ...
    save_path = M6_PROCESSED_RAW_DIR
    save_path_hypo = M6_PROCESSED_DIR
    # Create directories if they don't exist
    save_path.mkdir(parents=True, exist_ok=True)
    save_path_hypo.mkdir(parents=True, exist_ok=True)

    data_path = M6_RESULTS_DIR
    ...
    for count, file_name in enumerate(tqdm(file_name_all), start=start_idx):
        ...
        out_path = save_path / f'm6_raw_r_sim_{count}.npy'
        if out_path.exists():
            continue
    ...
    if len(hypocenter_all) > 0:
        ...
        out_hypo_name = f"m6_hypo_{start_idx}_{end_idx}.npy"
        np.save(save_path_hypo / out_hypo_name, hypo_all)


def test_processed_sample(
    processed_dir: Path = M6_PROCESSED_RAW_DIR, #change to raw m6_256_128, 320_160, etc
    save_dir: Path = M6_PLOTS_DIR,
    sample_index: int = 0,
    sample_index_list: list[int] | None = [0, 100, 1200, 1800],
    hypo_path: Path = M6_PROCESSED_DIR / 'm6_hypo_all.npy',
):
    ...
    - hypo_path: path to saved hypocenter array (m6_hypo_all.npy)
    """
...
def merge_hypocenters(
    hypo_dir: Path = M6_PROCESSED_DIR,
    part_files: list[str] | None = None,
    out_name: str = 'm6_hypo_all.npy',
):
    """
    Merge batch hypocenter files into one file.
    Default expects:
      - m6_hypo_0_1000.npy
      - m6_hypo_1000_2000.npy
    and saves as 'm6_hypo_all.npy' (per user request).
    """
    if part_files is None:
        part_files = ['m6_hypo_0_1000.npy', 'm6_hypo_1000_2000.npy']
    arrays = []
...
if __name__ == "__main__":
    ...
    if mode == "main":
        ...
        # Optional: python rupture_process_M6_all.py merge [out_name] [part1] [part2] ...
        out = sys.argv[2] if len(sys.argv) > 2 else 'm6_hypo_all.npy'
        parts = sys.argv[3:] if len(sys.argv) > 3 else None
        merge_hypocenters(out_name=out, part_files=parts)
    else:
        print("Usage: python rupture_process_M6_all.py [main|test|merge]")
