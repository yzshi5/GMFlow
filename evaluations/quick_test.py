...
post_fix = "final_video_M6" # 1c_add, 1c_add_tr
...
def _ensure_paths_configured() -> None:
    ...
    bad = [name for name, path in unresolved if "path/to/your" in str(path)]
    if bad:
        raise ValueError(
            "Please set path roots in this file or env vars before running. "
            "Required: GMFLOW_DATA_ROOT and GMFLOW_MODEL_ROOT. "
...
def end_to_end_test(sample_idx: int) -> Dict[str, Any]:
    ...
    # for better visulaization (match the coordinate)
